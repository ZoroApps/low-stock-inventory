...
test('main low stock block adapts to collection and cart pages', () => {
  ...
  assert.match(block, /placeRootNearProductPrice\(\);\s*retryProductPlacement\(\);\s*if \(portal\) portal\.hidden = true;\s*const portalPlaced = false;\s*message\.textContent = copy/);
